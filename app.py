...
# ── App Setup ─────────────────────────────────────────────────────────────────
app = Flask(__name__)
app.secret_key = "ai_student_secret_2024"
...
SMTP_PORT       = 587
SENDER_EMAIL    = "abhishekdau9555@gmail.com"
...
# ── ML Model — Synthetic Data (No CSV needed on server) ──────────────────────
def create_and_train_model():
    """
    Agar students.csv mile toh use karo,
    warna synthetic data se model train karo.
    Railway par CSV nahi hoti — isliye ye fallback zaroori hai.
    """
    ...
    if os.path.exists(csv_path):
        ...
        internal      = np.random.randint(30, 100, n)
        assignment    = np.random.randint(30, 100, n)
        maths         = np.random.randint(20, 100, n)
        cn            = np.random.randint(20, 100, n)
        ml            = np.random.randint(20, 100, n)
        communication = np.random.randint(30, 100, n)

        avg = (attendance + internal + assignment + maths + cn + ml + communication) / 7
        result = ["Pass" if (a >= 75 and avg >= 50) else "Fail"
                  for a, avg in zip(attendance, avg)]
...
def compute_placement_score(internal, assignment, maths, cn, ml,
    ...
    placement_score = round(
        ...
        (consistency_score    * 0.15), 1
    )
    placement_score = min(max(placement_score, 0), 100)

    if placement_score >= 85:
        category = "Excellent"; color = "#198754"; badge = "🟢"
        advice = "Placement Ready! Start applying to top companies."
    elif placement_score >= 70:
        category = "Good"; color = "#0d6efd"; badge = "🔵"
        advice = "Almost Ready. Polish your weaker areas."
    elif placement_score >= 50:
        category = "Needs Improvement"; color = "#ffc107"; badge = "🟡"
        advice = "Focus on technical skills and mock interviews."
    else:
        category = "Not Ready"; color = "#dc3545"; badge = "🔴"
        advice = "Dedicate serious time to technical and communication skills."
    ...
    return {
        "score": placement_score, "category": category,
        "color": color, "badge": badge, "advice": advice,
        "weakest_area": weakest_area, "components": components,
    }
...
def calculate_dashboard_stats():
    ...
    placement_scores = [r["placement_score"] for r in history if r.get("placement_score") is not None]
...
def generate_pdf(student_data: dict) -> str:
    ...
    s_wt  = st("WT",  fontSize=18, textColor=colors.white, alignment=TA_CENTER, fontName="Helvetica-Bold")
    s_ws  = st("WS",  fontSize=10, textColor=colors.HexColor("#cce0ff"), alignment=TA_CENTER)
    s_sec = st("SEC", fontSize=13, textColor=colors.HexColor("#1a3a5c"), fontName="Helvetica-Bold", spaceBefore=12, spaceAfter=4)
    s_lbl = st("LBL", fontName="Helvetica-Bold", textColor=colors.HexColor("#1a3a5c"), fontSize=10)
    s_val = st("VAL", fontSize=10)
    s_ctr = st("CTR", fontSize=10, alignment=TA_CENTER)
    s_adv = st("ADV", fontSize=11, fontName="Helvetica-Bold", textColor=colors.HexColor("#7d4e00"))
    s_hdr = st("MH",  fontName="Helvetica-Bold", textColor=colors.white, alignment=TA_CENTER, fontSize=10)

    # Header
    hdr = Table([[Paragraph("AI Based Student Prediction &amp; Counselling System", s_wt)],
                 [Paragraph("Automated Academic Performance Report", s_ws)]], colWidths=[cw])
    ...
    # Student Info
    story.append(Paragraph("Student Information", s_sec))
    ...
    info_data = [[Paragraph(r[0],s_lbl), Paragraph(str(r[1]),s_val),
                  Paragraph(r[2],s_lbl), Paragraph(str(r[3]),s_val)] for r in info_rows]
    iw = (cw - 3.5*cm) / 4
    ...
    photo_cell = Paragraph("No Photo", s_ctr)
    pf = student_data.get("photo","")
    if pf:
        pp = os.path.join(UPLOAD_FOLDER, pf)
        if os.path.exists(pp):
            try: photo_cell = RLImage(pp, width=2.8*cm, height=3.2*cm)
            except: pass
    combo = Table([[info_tbl, photo_cell]], colWidths=[cw-3.5*cm, 3.5*cm])
    combo.setStyle(TableStyle([("VALIGN",(0,0),(-1,-1),"TOP"),("LEFTPADDING",(1,0),(1,0),10)]))
    story += [combo, Spacer(1, 0.3*cm)]

    # Marks Table
    story.append(Paragraph("Academic Marks", s_sec))
    story.append(HRFlowable(width="100%", thickness=1, color=colors.HexColor("#1a3a5c"), spaceAfter=6))
    def row_bg(v):
        if v >= 75: return colors.HexColor("#d4edda")
        if v >= 50: return colors.HexColor("#fff3cd")
    ...
    for subj, val in subjects:
        ...
        m_data.append([Paragraph(subj,s_val), Paragraph(str(val),s_ctr),
                        Paragraph("100",s_ctr), Paragraph(status,s_ctr)])
    ...
    # Result Summary
    story.append(Paragraph("Result Summary", s_sec))
    story.append(HRFlowable(width="100%", thickness=1, color=colors.HexColor("#1a3a5c"), spaceAfter=6))
    ...
    rk_color = (colors.HexColor("#f8d7da") if "High" in risk else
                colors.HexColor("#fff3cd") if "Medium" in risk else colors.HexColor("#d4edda"))
    s_rv = st("RV", fontSize=14, fontName="Helvetica-Bold",
              textColor=colors.HexColor("#198754") if result=="Pass" else colors.HexColor("#dc3545"),
              alignment=TA_CENTER)
    sw = cw / 4
    sum_tbl = Table([
        [Paragraph("Prediction",s_lbl), Paragraph(result,s_rv),
         Paragraph("Percentage",s_lbl), Paragraph(f"{percentage}%",s_ctr)],
        [Paragraph("Grade",s_lbl), Paragraph(grade,s_ctr),
         Paragraph("Risk Level",s_lbl), Paragraph(risk,s_ctr)],
    ], colWidths=[sw*1.1, sw*0.9, sw*1.1, sw*0.9])
    sum_tbl.setStyle(TableStyle([
        ("GRID",(0,0),(-1,-1), 0.5, colors.HexColor("#cccccc")),
        ("BACKGROUND",(0,0),(0,-1), colors.HexColor("#eaf0fb")),
        ("BACKGROUND",(2,0),(2,-1), colors.HexColor("#eaf0fb")),
        ("BACKGROUND",(1,0),(1,0), r_color), ("BACKGROUND",(3,1),(3,1), rk_color),
    ...
    bar_color = (colors.HexColor("#198754") if percentage >= 75 else
                 colors.HexColor("#ffc107") if percentage >= 50 else colors.HexColor("#dc3545"))
    filled_w  = max(cw * (percentage / 100), 1.2*cm)
    s_pct     = st("PCT", fontSize=9, textColor=colors.white, fontName="Helvetica-Bold")
    bar_fill  = Table([[Paragraph(f"  {percentage}%", s_pct)]], colWidths=[filled_w], rowHeights=[18])
    bar_fill.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,-1), bar_color),
                                   ("TOPPADDING",(0,0),(-1,-1), 2), ("BOTTOMPADDING",(0,0),(-1,-1), 0)]))
    story += [Paragraph("Overall Performance", s_lbl), Spacer(1, 0.1*cm), bar_fill, Spacer(1, 0.3*cm)]

    # Placement Score
    ps = student_data.get("placement_score")
    pc = student_data.get("placement_category", "—")
    if ps is not None:
        story.append(Paragraph("Placement Readiness Score", s_sec))
        story.append(HRFlowable(width="100%", thickness=1, color=colors.HexColor("#1a3a5c"), spaceAfter=6))
        p_color = (colors.HexColor("#198754") if ps >= 85 else
                   colors.HexColor("#0d6efd") if ps >= 70 else
                   colors.HexColor("#ffc107") if ps >= 50 else colors.HexColor("#dc3545"))
        s_ps = st("PSCORE", fontSize=20, fontName="Helvetica-Bold", textColor=p_color, alignment=TA_CENTER)
        s_pc = st("PCAT",   fontSize=11, fontName="Helvetica-Bold", textColor=colors.white, alignment=TA_CENTER)
        score_cell = Table([[Paragraph(f"{ps}", s_ps)], [Paragraph("out of 100", s_ctr)]], colWidths=[3.5*cm])
        score_cell.setStyle(TableStyle([
            ("BACKGROUND",(0,0),(-1,-1), colors.HexColor("#eaf0fb")),
            ("BOX",(0,0),(-1,-1), 1, p_color),
            ("TOPPADDING",(0,0),(-1,-1), 8), ("ALIGN",(0,0),(-1,-1),"CENTER"),
        ]))
        cat_badge = Table([[Paragraph(pc, s_pc)]], colWidths=[cw - 3.5*cm])
        cat_badge.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,-1), p_color),
                                        ("TOPPADDING",(0,0),(-1,-1), 10), ("BOTTOMPADDING",(0,0),(-1,-1), 10)]))
        p_row = Table([[score_cell, cat_badge]], colWidths=[3.5*cm, cw - 3.5*cm])
        p_row.setStyle(TableStyle([("VALIGN",(0,0),(-1,-1),"MIDDLE"), ("LEFTPADDING",(1,0),(1,0), 10)]))
        story += [p_row, Spacer(1, 0.3*cm)]

    # Advice Box
    advice = student_data.get("advice","—")
    story.append(Paragraph("Counselling Advice", s_sec))
    story.append(HRFlowable(width="100%", thickness=1, color=colors.HexColor("#1a3a5c"), spaceAfter=6))
    adv_tbl = Table([[Paragraph(f"  {advice}", s_adv)]], colWidths=[cw])
    ...
    if gp and os.path.exists(gp):
        story.append(Paragraph("Marks Analysis Graph", s_sec))
        story.append(HRFlowable(width="100%", thickness=1, color=colors.HexColor("#1a3a5c"), spaceAfter=6))
        try: story.append(RLImage(gp, width=cw, height=6*cm))
        except: pass
    ...
    def add_footer(cv, d):
        ...
        cv.drawCentredString(width/2, 1.0*cm,
            f"AI Student Prediction System  |  Generated on {time.strftime('%d %B %Y at %I:%M %p')}  |  Confidential")
...
def send_email_notification(student_data: dict, pdf_path: str = None) -> dict:
    ...
    subject = (f"⚠️ URGENT: Attendance Alert for {name}" if is_high_risk
               else f"📊 Your Academic Result — {name}")
    result_color = "#198754" if result == "Pass" else "#dc3545"
    risk_banner  = (f"<div style='background:#f8d7da;border-left:5px solid #dc3545;"
                    f"padding:14px 18px;border-radius:8px;margin-bottom:18px;'>"
                    f"<strong>⚠️ High Risk Alert:</strong> Immediate counselling recommended.</div>"
                    ) if is_high_risk else ""

    html_body = f"""<html><body style="font-family:Arial,sans-serif;background:#f0f4f8;padding:24px;">
    <div style="max-width:600px;margin:auto;background:white;border-radius:14px;overflow:hidden;">
    <div style="background:#1a3a5c;color:white;padding:24px;text-align:center;">
    <h2 style="margin:0;">🎓 AI Student Prediction System</h2></div>
    <div style="padding:24px;"><p>Dear <strong>{name}</strong>,</p>{risk_banner}
    <table style="width:100%;border-collapse:collapse;margin:16px 0;">
    <tr><td style="padding:10px;background:#eaf0fb;font-weight:bold;">Prediction</td>
    <td style="padding:10px;color:{result_color};font-weight:bold;">{result}</td></tr>
    <tr><td style="padding:10px;background:#eaf0fb;font-weight:bold;">Percentage</td>
    <td style="padding:10px;">{percentage}%</td></tr>
    <tr><td style="padding:10px;background:#eaf0fb;font-weight:bold;">Grade</td>
    <td style="padding:10px;">{grade}</td></tr>
    <tr><td style="padding:10px;background:#eaf0fb;font-weight:bold;">Risk</td>
    <td style="padding:10px;">{risk}</td></tr></table>
    <div style="background:#fff3cd;border:1px solid #ffc107;border-radius:8px;padding:14px 18px;">
    💡 <strong>{advice}</strong></div>
    <p style="margin-top:24px;">Regards,<br><strong>AI Student Prediction System</strong></p>
    </div></div></body></html>"""
    ...
    try:
        ...
        return {"sent": False, "error": "Authentication failed. Use an App Password."}
...
def home():
    ...
    ctx = dict(
        ...
        placement=None, email_status=None,
    ...
    if request.method == "POST":
        try:
            ...
            result     = model.predict([[attendance, internal, assignment, maths, cn, ml, communication]])[0]
            percentage = min(round((attendance+internal+assignment+maths+cn+ml+communication)/7, 2), 100.0)
            grade      = compute_grade(percentage)
            advice     = compute_advice(attendance, maths, result)
            risk       = compute_risk(attendance)
            placement  = compute_placement_score(internal, assignment, maths, cn, ml, communication, attendance)
            ...
            record = {
                ...
                "result": result, "risk": risk, "percentage": percentage, "grade": grade,
                "attendance": attendance, "internal": internal, "assignment": assignment,
                "maths": maths, "cn": cn, "ml": ml, "communication": communication,
            ...
            try:
                ...
                ctx["error"] = f"DB error: {db_err}"

            if send_email_flag:
                if ctx["email"]:
                    pdf_path     = generate_pdf(record)
                    email_result = send_email_notification(record, pdf_path)
                    ctx["email_status"] = ({"ok": True, "msg": f"Email sent to {ctx['email']}."}
                                           if email_result["sent"]
                                           else {"ok": False, "msg": email_result["error"]})
                else:
                    ctx["email_status"] = {"ok": False, "msg": "No email address provided."}
